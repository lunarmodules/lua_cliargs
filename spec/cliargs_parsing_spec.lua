...
describe("Testing cliargs library parsing commandlines", function()
  ...
  it("tests option using -short-key value notation", function()
    cli:add_option("-out VALUE", "output file")
  ...
  it("tests unknown option", function()
    cli:set_name('unknown')
    populate_optionals(cli)
    local result, err = cli:parse({ "--foo=bar" }, true --[[no print]])
    assert.is.falsy(result)
    assert.is.equal("unknown: error: unknown/bad option: --foo=bar; re-run with --help for usage.", err)
  end)

  it("tests unknown flag", function()
    cli:set_name('unknown')
    populate_optionals(cli)
    local result, err = cli:parse({ "--foo" }, true --[[no print]])
    assert.is.falsy(result)
    assert.is.equal("unknown: error: unknown/bad flag: --foo; re-run with --help for usage.", err)
  end)
